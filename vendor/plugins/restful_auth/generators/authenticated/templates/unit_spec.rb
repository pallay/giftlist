...
<% end %>  end
  ...
  it 'requires username' do
    lambda do
      u = create_<%= file_name %>(:username => nil)
  ...
  it 'does not rehash password' do
    <%= table_name %>(:quentin).update_attributes(:username => 'quentin2')
...
protected
  def create_<%= file_name %>(options = {})
    <%= class_name %>.create({ :username => 'quire', :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
  end
end
